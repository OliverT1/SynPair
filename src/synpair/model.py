...
class SupConDualEncoder(DualEncoder):
    """
    Dual encoder with Supervised Contrastive loss (Khosla et al., 2020).
    Positives = diagonal *and* any other row whose label==1.
    Explicit fakes remain negatives.
    """

    def __init__(self, temperature=0.07, *args, **kw):
        super().__init__(*args, **kw)
        self.log_tau = nn.Parameter(torch.log(torch.tensor(temperature)))

    def supcon_loss(self, q, k, labels):
        """
        q, k : (N,D) L2‑normalised embeddings
        labels : (N,) 0/1; 1 = true pair
        For each anchor i, all j with labels[j]==1 are treated as positives.
        """
        sim = q @ k.T / torch.exp(self.log_tau)  # (N,N)
        logits = sim - torch.max(sim, dim=1, keepdim=True)[0]  # stability
        # mask of positives (including itself if label==1)
        pos_mask = (labels.unsqueeze(1) * labels.unsqueeze(0)).bool()
        exp_logits = torch.exp(logits) * (
            ~torch.eye(sim.size(0), device=sim.device).bool()
        )
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True) + 1e-8)
        loss = -(pos_mask * log_prob).sum(1) / (pos_mask.sum(1) + 1e-9)
        return loss.mean()

    # ---- override training_step ----
    def training_step(self, batch, _):
        vh_tok, vl_tok, labels = batch
        q, k = self(vh_tok, vl_tok)
        loss = self.supcon_loss(q, k, labels.to(self.device))
        self.log("train_loss", loss, prog_bar=True)
        return loss
